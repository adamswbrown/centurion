...
export function ClientAppointmentsCalendar() {
  ...
  const appointmentsByDay = useMemo(() => {
    const map = new Map<string, typeof appointmentList>()
    appointmentList.forEach((appointment) => {
      const dayKey = format(new Date(appointment.startTime), "yyyy-MM-dd")
      const existing = map.get(dayKey) ?? []
      map.set(dayKey, [...existing, appointment])
    })
    return map
  }, [appointmentList])
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            {appointmentList.length} appointment(s)
          </p>
        </div>
